...
interface ResumenData {
  ...
  ultimos_movimientos: {
    ...
    fecha_hora: string; usuario: { nombre_completo: string }; expediente: { codigo: string };
  }[];
}
...
export default function AuditoriaPage() {
  ...
  const cargarDatos = async () => {
    setCargando(true);
    try { await Promise.all([cargarResumen(), cargarBitacora(1)]); }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-1 border-b border-gray-200">
        {([{ key: 'resumen', label: 'Resumen', icon: <TrendingUp size={13} /> }, { key: 'bitacora', label: 'Bitácora BD', icon: <Shield size={13} /> }] as const).map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors flex items-center gap-1.5 ${tab === t.key ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
            {t.icon}{t.label}
          </button>
        ))}
      </div>
      ...
      {tab === 'resumen' && resumen && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <Card><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-lg bg-blue-100 flex items-center justify-center"><Activity size={18} className="text-blue-600" /></div><div><p className="text-2xl font-bold text-gray-900">{resumen.hoy.expedientes_registrados}</p><p className="text-xs text-gray-500">Expedientes registrados hoy</p></div></div></Card>
            <Card><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-lg bg-green-100 flex items-center justify-center"><TrendingUp size={18} className="text-green-600" /></div><div><p className="text-2xl font-bold text-gray-900">{resumen.hoy.pagos_verificados}</p><p className="text-xs text-gray-500">Pagos verificados hoy</p></div></div></Card>
          </div>
          <Card>
            ...
            <div className="mt-3 space-y-2">
              ...
              {resumen.expedientes_por_estado.length === 0 && <p className="text-sm text-gray-400 text-center py-4">Sin expedientes activos</p>}
            </div>
          </Card>
          <Card>
              ...
              {resumen.ultimos_movimientos.map((mov, idx) => (
                <div key={idx} className="flex items-start gap-3 text-sm border-b border-gray-50 pb-2 last:border-0">
                  <div className="flex-1 min-w-0">
                    <p className="text-gray-700"><span className="font-mono text-blue-600 text-xs">{mov.expediente.codigo}</span>{' — '}<span className="text-gray-600">{mov.comentario ?? mov.tipo_accion}</span></p>
                    <p className="text-xs text-gray-400 mt-0.5">{mov.usuario.nombre_completo} · {formatFechaHora(mov.fecha_hora)}</p>
                  </div>
          ...
          <Card>
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <select value={filtroTabla} onChange={(e) => setFiltroTabla(e.target.value)} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg outline-none focus:border-blue-500">
              ...
              <div>
                ...
                <select value={filtroOperacion} onChange={(e) => setFiltroOperacion(e.target.value)} className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg outline-none focus:border-blue-500">
                  <option value="">Todas</option>
            ...
            <div className="flex justify-end mt-3"><Button size="sm" onClick={() => cargarBitacora(1)}>Filtrar</Button></div>
          </Card>
          <div className="flex items-center justify-between">
            ...
            <Table keyField="id" data={registros} emptyText="No hay registros de auditoría"
              columns={[
                ...
                { key: 'cambio', header: 'Cambio de estado', render: (r) => {
                  ...
                  if (antes && despues && antes !== despues) return <div className="flex items-center gap-1 text-xs"><EstadoBadge estado={antes as EstadoExpediente} size="sm" /><span className="text-gray-400">→</span><EstadoBadge estado={despues as EstadoExpediente} size="sm" /></div>;
