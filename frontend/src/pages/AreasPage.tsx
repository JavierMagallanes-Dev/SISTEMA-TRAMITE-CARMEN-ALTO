...
export default function AreasPage() {
  ...
  const cargarBandeja = async () => {
    ...
    try {
      ...
      toast.error({ titulo: 'Error al cargar la bandeja.' });
    } finally {
      setCargando(false);
    }
  };
  ...
  const handleTomar = async (exp: ExpedienteBandeja) => {
    ...
    try {
      ...
      toast.success({ titulo: 'Expediente tomado', descripcion: `${exp.codigo} en evaluación técnica.` });
      setConfirmTomar(null);
      cargarBandeja();
    } catch (err: any) {
      toast.error({ titulo: err?.response?.data?.error ?? 'Error al tomar.' });
  ...
  const handleObservar = async () => {
    ...
    try {
      ...
      toast.success({ titulo: 'Expediente observado', descripcion: `${expAccion.codigo} marcado como OBSERVADO.` });
  ...
  const handleRechazar = async () => {
    ...
    try {
      ...
      toast.success({ titulo: 'Expediente rechazado', descripcion: `${expAccion.codigo} rechazado.` });
      setModalRechazar(false);
      setComentario('');
      cargarBandeja();
    } catch (err: any) {
      toast.error({ titulo: err?.response?.data?.error ?? 'Error al rechazar.' });
    } finally { setLoading(false); }
  };
  ...
  const handleVistoBueno = async (exp: ExpedienteBandeja) => {
    ...
    try {
      ...
      toast.success({ titulo: 'Visto bueno otorgado', descripcion: `${exp.codigo} enviado al jefe.` });
      setConfirmVisto(null);
      cargarBandeja();
    } catch (err: any) {
      toast.error({ titulo: err?.response?.data?.error ?? 'Error al dar visto bueno.' });
    } finally { setLoading(false); }
  };
  ...
  const handleReactivar = async (exp: ExpedienteBandeja) => {
    ...
    try {
      ...
      toast.success({ titulo: 'Expediente reactivado', descripcion: `${exp.codigo} volvió a EN_PROCESO.` });
  ...
  const handleArchivoAdjuntoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 10 * 1024 * 1024)   { toast.warning({ titulo: 'El archivo no puede superar 10MB.' }); return; }
  ...
  const handleAdjuntar = async () => {
    ...
    try {
      ...
      toast.success({ titulo: 'Documento adjuntado', descripcion: `Documento añadido al expediente ${expAdjuntar.codigo}.` });
      setModalAdjuntar(false);
      setArchivoAdjunto(null);
    } catch (err: any) {
      toast.error({ titulo: err?.response?.data?.error ?? 'Error al adjuntar el documento.' });
  ...
  const handleSubirPdfFirmado = async () => {
    ...
    try {
      ...
      toast.success({ titulo: 'PDF firmado subido', descripcion: `Código: ${res.codigo_verificacion_firma}` });
      setModalPdf(false);
      setArchivoPdf(null);
      cargarBandeja();
    } catch (err: any) {
      toast.error({ titulo: err?.response?.data?.error ?? 'Error al subir el PDF firmado.' });
  ...
  const handleArchivar = async (exp: ExpedienteBandeja) => {
    ...
    try {
      ...
      toast.success({ titulo: 'Expediente archivado', descripcion: `${exp.codigo} archivado correctamente.` });
  ...
  return (
    <div className="p-6 space-y-6">
              ...
              <Card key={exp.id}>
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex flex-wrap gap-2 shrink-0">
                    ...
                    {esJefe && exp.estado === 'LISTO_DESCARGA' && <Button size="sm" icon={<Upload size={13} />} onClick={() => { setExpPdf(exp); setArchivoPdf(null); setModalPdf(true); }}>Subir PDF firmado</Button>}
        ...
        {cargandoDet ? <Spinner /> : detalle ? (
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><p className="text-xs text-gray-400">Ciudadano</p><p className="font-medium">{detalle.ciudadano.nombres} {detalle.ciudadano.apellido_pat}</p><p className="text-xs text-gray-500">DNI: {detalle.ciudadano.dni}</p></div>
              <div><p className="text-xs text-gray-400">Trámite</p><p className="font-medium">{detalle.tipoTramite.nombre}</p><p className="text-xs text-gray-500">{detalle.tipoTramite.plazo_dias} días de plazo</p></div>
            ...
            {detalle.url_pdf_firmado && (
              <div className="bg-green-50 rounded-lg p-3">
                ...
                <a href={detalle.url_pdf_firmado} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline flex items-center gap-1"><Download size={13} /> Descargar PDF firmado →</a>
                {detalle.codigo_verificacion_firma && <p className="text-xs text-gray-400 mt-1 font-mono">Código: {detalle.codigo_verificacion_firma}</p>}
              </div>
            )}
            <div><CardTitle>Historial de movimientos</CardTitle><div className="mt-3"><TimelineMovimientos movimientos={detalle.movimientos} /></div></div>
          </div>
        ) : null}
      </Modal>

      {/* Modal adjuntar */}
      <Modal open={modalAdjuntar} onClose={() => { setModalAdjuntar(false); setArchivoAdjunto(null); }} title="Adjuntar documento al expediente" size="sm"
        footer={<><Button variant="secondary" onClick={() => { setModalAdjuntar(false); setArchivoAdjunto(null); }}>Cancelar</Button><Button variant="primary" loading={loadingAdjunto} icon={<Paperclip size={14} />} onClick={handleAdjuntar} disabled={!archivoAdjunto}>Adjuntar documento</Button></>}>
        <div className="space-y-4">
          {expAdjuntar && <div className="bg-blue-50 rounded-lg p-3"><p className="text-xs text-gray-500">Expediente</p><p className="font-mono text-sm font-semibold text-blue-600">{expAdjuntar.codigo}</p><p className="text-sm text-gray-600">{expAdjuntar.tipoTramite.nombre}</p></div>}
          ...
          {archivoAdjunto ? (
            <div className="flex items-center gap-3 p-3 bg-green-50 border border-green-200 rounded-lg">
              ...
              <div className="flex-1 min-w-0"><p className="text-sm font-medium text-green-700 truncate">{archivoAdjunto.name}</p><p className="text-xs text-green-500">{(archivoAdjunto.size / 1024).toFixed(1)} KB</p></div>
            ...
            <div onClick={() => adjuntoRef.current?.click()} className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-colors">
      ...
      {/* Modal PDF firmado */}
      <Modal open={modalPdf} onClose={() => { setModalPdf(false); setArchivoPdf(null); }} title="Subir PDF firmado con FirmaPeru" size="sm"
        footer={<><Button variant="secondary" onClick={() => { setModalPdf(false); setArchivoPdf(null); }}>Cancelar</Button><Button variant="primary" loading={loadingPdf} icon={<Upload size={14} />} onClick={handleSubirPdfFirmado} disabled={!archivoPdf}>Subir PDF firmado</Button></>}>
        <div className="space-y-4">
          <Alert type="info" message="Firma el PDF con FirmaPeru usando tu DNI electrónico y luego súbelo aquí." />
          {archivoPdf ? (
            <div className="flex items-center gap-3 p-3 bg-green-50 border border-green-200 rounded-lg">
              <FileText size={16} className="text-green-600 shrink-0" />
              <div className="flex-1 min-w-0"><p className="text-sm font-medium text-green-700 truncate">{archivoPdf.name}</p><p className="text-xs text-green-500">{(archivoPdf.size / 1024).toFixed(1)} KB</p></div>
            ...
            <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-colors">
              <Upload size={24} className="mx-auto text-gray-400 mb-2" />
      ...
      {/* Confirms */}
      <ConfirmModal open={!!confirmTomar} onClose={() => setConfirmTomar(null)} onConfirm={() => confirmTomar && handleTomar(confirmTomar)} title="Tomar expediente" message={`¿Tomar el expediente ${confirmTomar?.codigo}?`} confirmText="Tomar" loading={loading} />
      <ConfirmModal open={!!confirmVisto} onClose={() => setConfirmVisto(null)} onConfirm={() => confirmVisto && handleVistoBueno(confirmVisto)} title="Dar visto bueno" message={`¿Confirmas el visto bueno para ${confirmVisto?.codigo}?`} confirmText="Dar visto bueno" loading={loading} />
      <ConfirmModal open={!!confirmArchivar} onClose={() => setConfirmArchivar(null)} onConfirm={() => confirmArchivar && handleArchivar(confirmArchivar)} title="Archivar expediente" message={`¿Archivar permanentemente ${confirmArchivar?.codigo}?`} confirmText="Archivar" loading={loading} danger />
      <ConfirmModal open={!!confirmReactivar} onClose={() => setConfirmReactivar(null)} onConfirm={() => confirmReactivar && handleReactivar(confirmReactivar)} title="Reactivar expediente" message={`¿Confirmas que el expediente ${confirmReactivar?.codigo} ha sido subsanado?`} confirmText="Reactivar" loading={loading} />
    </div>
  );
}