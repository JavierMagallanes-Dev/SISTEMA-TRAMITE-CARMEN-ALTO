...
export default function MesaPartesPage() {
  ...
  const handleObservar = async () => {
    ...
    try {
      ...
      setSuccess(`Expediente ${expObservar.codigo} marcado como OBSERVADO. Ciudadano notificado.`);
  ...
  const handleReactivar = async () => {
    ...
    try {
      ...
      setSuccess(`Expediente ${detalle.codigo} reactivado. Ya puedes derivarlo.`);
      setModalDetalle(false);
      cargarDatos();
    } catch (err: any) {
  ...
  const handleRegistrar = async () => {
    ...
    try {
      ...
      setSuccess(`Expediente ${res.expediente.codigo} registrado correctamente.`);
  ...
  const handleArchivoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (file.type !== 'application/pdf') { setError('Solo se aceptan PDFs.'); return; }
  ...
  // Estados en los que MDP puede observar
    ...
    <div className="p-6 space-y-6">
          ...
          <Table keyField="id" data={bandeja} emptyText="No hay expedientes en bandeja."
            columns={[
              { key: 'codigo', header: 'Código', render: (r) => <span className="font-mono text-xs text-blue-600 font-semibold">{r.codigo}</span> },
              {
                ...
                render: (r) => <div><p className="text-sm font-medium text-gray-800">{r.ciudadano.nombres} {r.ciudadano.apellido_pat}</p><p className="text-xs text-gray-400">{r.ciudadano.dni}</p></div>,
              },
              { key: 'estado', header: 'Estado', render: (r) => <EstadoBadge estado={r.estado} size="sm" /> },
              {
                ...
                render: (r) => {
                  ...
                  return <span className={`text-xs font-medium ${colorDiasRestantes(dias)}`}>{dias < 0 ? `Vencido ${Math.abs(dias)}d` : `${dias}d restantes`}</span>;
                },
              },
              {
          ...
          <div className="mt-4 space-y-4">
            ...
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1">
                Documento adjunto (PDF) <span className="text-gray-400 font-normal">— opcional</span>
              </label>
              {archivoPdf ? (
      ...
      <Modal open={modalDetalle} onClose={() => setModalDetalle(false)} title="Detalle del expediente" size="lg">
        {cargandoDet ? <Spinner text="Cargando detalle..." /> : detalle ? (
          <div className="space-y-5">
            ...
            {/* Acciones para Mesa de Partes */}
            <div className="space-y-2">
              {/* Botón Observar — solo si el estado lo permite */}
              {puedeObservar(detalle.estado) && (
                <div className="pt-2 border-t border-gray-100">
                  <Button
                    variant="secondary"
                    icon={<AlertCircle size={14} />}
                    onClick={abrirObservar}
                    className="w-full justify-center"
                  >
                    Observar expediente — Faltan documentos
                  </Button>
                  <p className="text-xs text-gray-400 text-center mt-1">
                    Marca el expediente como observado y notifica al ciudadano
                  </p>
                </div>
              )}

              {/* Botón Reactivar — Solo si está OBSERVADO */}
              {detalle.estado === 'OBSERVADO' && (
                <div className="pt-2 border-t border-gray-100">
                  <Button
                    ...
                    className="w-full justify-center"
                  >
                    Reactivar expediente — Documentos subsanados
                  </Button>
                  <p className="text-xs text-gray-400 text-center mt-1">
                    El expediente volverá a RECIBIDO y podrás derivarlo
                  </p>
                </div>
              )}
            </div>
      ...
      <Modal open={modalObservar} onClose={() => setModalObservar(false)} title="Observar expediente" size="sm"
        footer={<>
          ...
          <Button variant="secondary" icon={<AlertCircle size={14} />} loading={loadingObservar}
            onClick={handleObservar} disabled={!comentarioObs.trim()}
      ...
      >
        <div className="space-y-4">
          ...
          <Input
            ...
            placeholder="Ej: Falta fotocopia del DNI, plano de ubicación, certificado de..."
            value={comentarioObs}
      ...
      >
        {expDerivar && (
          <div className="space-y-4">
            ...
            <Input label="Instrucciones (opcional)" placeholder="Instrucciones para el área técnica..."
      ...
      >
        <div className="space-y-4">
          ...
          <Input label="Token (opcional)" placeholder="O confirma directamente con el botón"
